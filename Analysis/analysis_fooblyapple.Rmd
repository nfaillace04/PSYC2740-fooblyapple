...
# We're very confused why there is no data in the middle.
...
emmeans::emmeans(rt_model,
                 pairwise ~ valence,
                 adjust = "tukey")
#Follow-up tests indicate that there is no significant difference between response times in any condition. There is no comparison where (p < 0.001).
```
#Meaning Check Sentiment
```{r}
mc_sentiment_score = sentiment(meaning_check$response)

meaning_check_sentiment = meaning_check %>%
  select(ID, condition, response, cue) %>%
  cross_join(mc_sentiment_score) #I don't know where to go from here to incorporate valence based on condition in order to graph responses and proximity to the intended valence.
```
