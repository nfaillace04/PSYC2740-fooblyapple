...
incomplete_participants = final_data %>%
  filter(typeoftrial == "target") %>%
  group_by(ID) %>%
  summarize(total_trials = n()) %>%
  filter(total_trials < 105) %>%
  length()
